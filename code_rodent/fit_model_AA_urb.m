function [dates, alpha1, alpha2, alpha3,alpha1hat, alpha2hat, alpha3hat,tau1, tau2, tau3, tau1hat, tau2hat, tau3hat, logbetarain,logbetatemp, betalongterm, Y0, GRhat, Y,Yhat,AAdensity1,AAhat, r2_logAA, r2_AA, r2_GR, exitflag] = fit_model_AA_urb(dataset, nknots, h, penalty, saveBool);
...
% calculate AAhat
AAhat = zeros(timePts, 1);
AAhat(3,1)=AAdensity1(3,1);
for i = 4:timePts
    AAhat(i,1)=AAhat(i-1,1)*exp(GRhat(i,1));
end
...
SSR2 = sum((AAhat-mean(AAdensity1)).^2);
SSTO2 = sum((AAdensity1-mean(AAdensity1)).^2);
r2_AA = SSR2/SSTO2;
...
if saveBool
    filename = strcat('Opt_Results_AA_urb_', dataset);
    save(filename, 'nknots', 'm', 'n', 'penalty', 'h', 'dates', 'Wblt', 'Iblt', 'X', 'Y0', 'GRhat','Y','Yhat','AAdensity1','AAhat', 'alpha1', 'alpha1hat', 'alpha2', 'alpha2hat', 'alpha3', 'alpha3hat', 'tau1', 'tau1hat','tau2', 'tau2hat','tau3', 'tau3hat','logbetarain','logbetatemp','betalongterm', 'f2hat');
end