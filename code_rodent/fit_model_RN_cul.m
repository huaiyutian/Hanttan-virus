function [dates, alpha1, alpha2, alpha3,alpha1hat, alpha2hat, alpha3hat, tau1, tau2, tau3, tau1hat, tau2hat, tau3hat, logbetarain,logbetatemp, betalongterm, Y0, GRhat,Y,Yhat,RNdensity1,RNhat,  r2_logRN,r2_RN,r2_GR, exitflag2] = fit_model_RN_cul(dataset, nknots, h, penalty, saveBool)
% h=1;penalty=-5;nknots = 10;   
load(dataset);
avg_patch_size = avg_cul_patch;
...
% calculate RNhat
RNhat = zeros(timePts, 1);
RNhat(5,1)=RNdensity1(5,1);
for i = 6:timePts
    RNhat(i,1)=RNhat(i-1,1)*exp(GRhat(i,1));
end
...
SSR2 = sum((RNhat-mean(RNdensity1)).^2);
SSTO2 = sum((RNdensity1-mean(RNdensity1)).^2);
r2_RN = SSR2/SSTO2;
...
if saveBool
    filename = strcat('Opt_Results_RN_cul_', dataset);
    save(filename, 'nknots', 'm', 'n', 'penalty', 'h', 'dates', 'Wblt', 'Iblt', 'X', 'Y0', 'GRhat','Y','Yhat','RNdensity1','RNhat', 'alpha1', 'alpha1hat', 'alpha2', 'alpha2hat', 'alpha3', 'alpha3hat', 'tau1', 'tau1hat','tau2', 'tau2hat','tau3', 'tau3hat','logbetarain','logbetatemp','betalongterm', 'f2hat');
